...
myModelRidreg<-ridgereg$new(formula=reformulate(colnames(trainData[,-14]),"medv"),
                            data=as.data.frame(trainData),lambda=1)

print(paste0("RMSE on Training Data with (Lambda=1): ",RMSE(myModelRidreg$predict(),trainData[,14])))


myModelRidreg<-ridgereg$new(formula=reformulate(colnames(trainData[,-14]),"medv"),
                            data=as.data.frame(trainData),lambda=2)

print(paste0("RMSE on Training Data with (Lambda=2): ",RMSE(myModelRidreg$predict(),trainData[,14])))
...
customRidgeReg$predict <- function(modelFit, newdata, preProc = NULL, submodels = NULL){
  print(dim(newdata))
  print(2131231)
  retult<-modelFit$predict(newData=newdata)
  cat(head(retult))
  print(213312321)
  return(retult)
...
train(y=as.numeric(trainData[,14]),
      x=trainData[,-14],
      method=customRidgeReg,
      verbose =TRUE,
      
      tuneGrid = expand.grid(lambda = seq(5, 13)))
...
myModelRidreg<-ridgereg$new(formula=reformulate(colnames(trainData[,-14]),"medv"),
                            data=as.data.frame(trainData),lambda=2)
myModelRidreg$predict(newData=as.data.frame(trainData))
```
